...
export class AuthService {
  ...
  async register(registerDto: RegisterDto) {
    ...
    if (await this.usersRepository.findOne({ where: { email: registerDto.email } })) {
  ...
  async login(loginDto: LoginDto) {
    ...
    const check_id_for = await this.refreshRepository.findOne({ where: { userId: user.id } });
    if (check_id_for?.userId === user.id) {
      await this.refreshRepository.update({ userId: user.id }, {
        token: refreshToken,
        expiresAt
      });
      return {
  ...
  async logout(userId: string, token?: string) {
    ...
    const refreshToken = await this.refreshRepository.findOne({ where: { userId } });
    if (!user) {
    ...
    if (token) {
      try {
        ...
        if (decoded) {
          // Thêm vào blacklist với thời gian tồn tại = thời gian còn lại của token
          const remainingTime = decoded - Math.floor(Date.now() / 1000);
          if (remainingTime > 0) {
            this.tokenBlacklist.add(token);

            // Tự động xóa khỏi blacklist sau khi hết hạn
            setTimeout(() => {
  ...
  async changePassword(
    ...
    // Verify old password
    const isPasswordValid = await bcrypt.compare(changePassDto.oldPassword, user.password);
    if (!isPasswordValid) {
